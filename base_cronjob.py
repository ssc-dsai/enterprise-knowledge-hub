...
TO TEST USING PSQL, we insert logs with a different dump_date in the metadata field, and check if the cronjob detects the new dump and updates the log accordingly:
...
VALUES ('cronjob-frwiki', 'New Dump Link Detected and Downloaded', '{"dump_date": "Wed, 07 Apr 2040 20:29:35 GMT"}', NOW());

INSERT INTO run_history (service_name, status, metadata, timestamp)
VALUES ('cronjob-enwiki', 'New Dump Link Detected and Downloaded', '{"dump_date": "Wed, 07 Apr 2040 20:29:35 GMT"}', NOW());
...
def wiki_check(wiki_dump_content_url, wiki_dump_index_url, dump_key):
    ...
    logging.info(f"Current timestamp: {datetime.datetime.now()}")

    latest_dump_date = run_history_repository.cronjob_get_most_recent_dump_date("cronjob-" + dump_key)
    logging.info(f"Latest dump date from DB: {latest_dump_date}")

    if latest_dump_date:
        logging.info(f"Latest stored dump date in our records: {latest_dump_date}")
    else:
    ...
    if published_date and string_published_date != latest_dump_date:
        logging.info(f"Link published date from the RSS feed: {string_published_date} is different from latest recorded dump date: {latest_dump_date}")
        wiki_dump_update.download_latest_dump(wiki_dump_content_url, wiki_dump_index_url)

        logging.info("inserting into DB")
        run_history_repository.cronjob_insert_new_log("cronjob-" + dump_key, RunStatus.DUMP_LINK_UPDATED, {"dump_date": string_published_date}, datetime.datetime.now())
