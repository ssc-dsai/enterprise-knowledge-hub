...
def download_latest_dump(wiki_dump_content_url, wiki_dump_index_url):
    ...
    print(f"List of download links retrieved: {list}")
        ...
        if response.status_code == 200:
            with open(f"{DOWNLOAD_DIRECTORY}/{filename}", "wb") as f:
                ...
                for chunk in response.iter_content(chunk_size=8192):
                    f.write(chunk)
            print(f"Successfully downloaded {filename} to {DOWNLOAD_DIRECTORY}, launching checksum verification...")
...
def checksum_verification(filename):
    ...
    print(f"Constructed MD5 URL: {md5_url}")
    ...
    if page.status_code == 200:
        ...
        if find_filename == -1:
            print(f"Filename {filename} not found in MD5 checksums, check to ensure it exists/ensure that dump was not",
...
def hashing_verification_md5(filename, good_hash):
    ...
    if calculated_hash == good_hash:
        print(f"MD5 hash verification successful for {filename}.")
    else:
        print(f"MD5 hash verification failed for {filename}. Expected: {good_hash}, Got: {calculated_hash}, deleting file...")
        os.remove(f"{DOWNLOAD_DIRECTORY}/{filename}")