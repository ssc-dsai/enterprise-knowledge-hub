"""PyTorch embedding backend utilities."""
...
class TorchEmbeddingBackend(EmbeddingBackendProvider):
    """Embedding backend using Hugging Face torch models."""
    ...
    def set_device(self, device: str) -> None:
        """Update the torch device used for inference."""
        self.device = device

    def embed_torch(self, texts: List[str]) -> np.ndarray:
        """Encode text and return mean-pooled embeddings."""

        # this means i'm retokenizing i think? doing it twice.
        # chunk token by text, tokenizes, then decodes back to text.
        # maybe don't decode.
        # check how long it takes, we can deal with this when we split
        # embed and tokenizing to ensure optimizing by time.
